...
- **`main.jar` is the cross-sibling name contract.** Every distributable zip ships its fat jar as `main.jar` at the zip root beside `dependencies/` (the `dist` layout — see [`../kzen/docs/RELEASING.md`](../kzen/docs/RELEASING.md)); nothing renames it at install time. `ProjectCreator` only *verifies* it is there (create and upgrade both `check` it, failing loudly on an archetype that lacks it), and kzen-shell's `MainJarRunner` looks for exactly that name — so the dist build, the launcher, and the shell all have to move together if it ever changes.
